...
const Timeline = () => {
  const timelineData = [
    ...
    { 
      ...
      details : "An online preliminary round to test your mettle. Make your Team and submit your idea and wait till we annouce the Shortlisted Teams" ,
    },
    { 
     
      title: " Acceptance Letter Rollout" ,
      date : "March 27 , 2025" ,
      details : "Your team is accepted to attend the Hackathon, how awesome is that? Make sure you have RSVP'd to attend the Finals " ,
    },
    { 
          ...
          {timelineData.map((item, index) => (
            <li key={index} className={visibleItems.includes(index) ? "in-view" : ""}>
              <div className=" flex flex-col   w-[450px] h-auto timeline-box">
                <h1 className=" text-3xl text-purple-700 font-bold mt-2">{item.title}</h1>
                <p className=" text-xl text-purple-500 font-bold p-1 mt-1">{item.date}</p>
                <p1 className="text-md text-gray-400 font-normal p-1 mt-1 ">{item.details}</p1>
              </div>
            </li>
          ))}
        </ul>
